...
BEGIN
  ...
  WITH scored AS (
    SELECT
      ...
      -- Deal ratio: valuation / price
      CASE WHEN a.price > 0 THEN ROUND(a.valuation / a.price, 2) ELSE 0 END AS _deal_ratio,
      -- Gem score computation (0-100)
      LEAST(100, GREATEST(0, (
        -- Deal score (30%): cap ratio at 10x for scoring
  ...
  SELECT
    (SELECT COUNT(*) FROM scored WHERE scored._gem_score >= p_min_gem_score)::bigint AS total_count,
    s._id,
    s._domain_name,
    s._price,
    s._valuation,
