...
CREATE OR REPLACE FUNCTION public.find_hidden_gems(
  p_min_gem_score integer DEFAULT 40,
  p_max_price numeric DEFAULT 500,
  p_tld_filter text DEFAULT 'com',
...
RETURNS TABLE(
  total_count bigint, id uuid, domain_name text, price numeric, valuation numeric,
  brandability_score smallint, pronounceability_score smallint, domain_age integer,
  bid_count integer, traffic_count integer, tld text, end_time timestamp with time zone,
  auction_type text, inventory_source text, trademark_risk text,
  deal_ratio numeric, gem_score integer, created_at timestamp with time zone, has_comparable boolean
)
...
BEGIN
  ...
  WITH gems AS (
    SELECT
      a.id AS _id, a.domain_name AS _domain_name, a.price AS _price, a.valuation AS _valuation,
      a.brandability_score AS _brandability_score, a.pronounceability_score AS _pronounceability_score,
      a.domain_age AS _domain_age, a.bid_count AS _bid_count, a.traffic_count AS _traffic_count,
      a.tld AS _tld, a.end_time AS _end_time, a.auction_type AS _auction_type,
      a.inventory_source AS _inventory_source, a.trademark_risk AS _trademark_risk,
      a.created_at AS _created_at,
      CASE WHEN a.price > 0 THEN ROUND(a.valuation / a.price, 2) ELSE 0 END AS _deal_ratio,
      COALESCE(a.gem_score::integer, 0) AS _gem_score,
      EXISTS (
        SELECT 1 FROM comparable_sales cs
        WHERE LOWER(REPLACE(cs.tld, '.', '')) = LOWER(REPLACE(a.tld, '.', ''))
        AND cs.sale_price > a.price LIMIT 1
      ) AS _has_comparable
    FROM auctions a
    WHERE a.valuation IS NOT NULL AND a.price > 0 AND a.valuation > a.price
      AND a.gem_score IS NOT NULL AND a.gem_score > 0
      ...
      AND (p_tld_filter IS NULL OR LOWER(REPLACE(a.tld, '.', '')) = LOWER(p_tld_filter))
  ),
  counted AS (SELECT COUNT(*) AS cnt FROM gems WHERE gems._gem_score >= p_min_gem_score)
  SELECT c.cnt, g._id, g._domain_name, g._price, g._valuation,
    g._brandability_score, g._pronounceability_score, g._domain_age,
    g._bid_count, g._traffic_count, g._tld, g._end_time,
    g._auction_type, g._inventory_source, g._trademark_risk,
    g._deal_ratio, g._gem_score, g._created_at, g._has_comparable
