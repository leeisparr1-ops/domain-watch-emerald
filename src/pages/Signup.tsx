...
export default function Signup() {
  ...
  const getSocialErrorMessage = (provider: "Google" | "Apple", message: string) => {
    const normalized = message.trim().toLowerCase();

    if (!message || message.trim() === "{}" || normalized === "[object object]") {
      return `${provider} sign-up failed. Please try again.`;
    }

    if (normalized.includes("cancel")) {
      return `${provider} sign-up was cancelled.`;
    }

    return `${provider} sign-up failed. ${message}`;
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-background relative overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 rounded-2xl bg-card border border-border relative z-10 animate-fade-in">
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <Button type="submit" variant="hero" className="w-full" disabled={loading || socialLoading !== null || !turnstileToken}>
        ...
        <p className="mt-4 text-xs text-center text-muted-foreground">
          Free tier includes 30 search patterns. Upgrade anytime for more.
        </p>
